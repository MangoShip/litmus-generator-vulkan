...
__kernel void litmus_test (
  ...
  if(shuffled_workgroup < stress_params[9]) {
    ...
    uint r0 = atomic_fetch_add_explicit(&wg_test_locations[y_0], 0, memory_order_relaxed);
    uint unused = atomic_exchange_explicit(&wg_test_locations[y_1], 1, memory_order_relaxed);
    atomic_work_item_fence(CLK_LOCAL_MEM_FENCE, memory_order_seq_cst, memory_scope_device);
